// Phase 3.8 — mesh-refinement verification for scalar diffusion.
//
// Independent convergence check for the analytical Poiseuille solution.
// The oracle is u(y) = G*y*(H-y)/(2*mu), with -mu*u'' = G.
// Cell-centre values are compared using volume-weighted L2/Linf norms.
...
double solve_error(std::size_t n)
{
    ...
    for (std::size_t c = 0; c < n; ++c)
        // Source is a volumetric density; the assembler multiplies it by V_c.\n        source[c] = G;
    ...
    if (result.linear_result.status != SolverStatus::CONVERGED)
        throw std::runtime_error("Poiseuille refinement solve did not converge");
    ...
    for (std::size_t c = 0; c < n; ++c) {
        ...
        const double exact = G * y * (H - y) / (2.0 * mu);
    ...
    if (!(sum_volume > 0.0)) throw std::runtime_error("invalid total volume");

    const double l2 = std::sqrt(sum_sq / sum_volume);
    EXPECT_TRUE(std::isfinite(l2));
    EXPECT_TRUE(std::isfinite(linf));
    return l2;
}
...
int main()
{
    ...
    for (const auto n : levels) {
        const double error = solve_error(n);
        errors.push_back(error);
        std::cout << "Poiseuille refinement N=" << n
                  << " L2=" << error << "\n";
    ...
    for (std::size_t i = 1; i < errors.size(); ++i) {
        const double order =
            std::log(errors[i - 1] / errors[i]) / std::log(2.0);
        std::cout << "Poiseuille observed order "
                  << levels[i - 1] << "->" << levels[i]
                  << ": " << order << "\n";
        EXPECT_TRUE(std::isfinite(order));
