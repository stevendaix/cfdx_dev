...
struct Comparison { double rms = 0.0; double max_abs = 0.0; };
struct CavityResult {
...
CavityResult solve_cavity(const CavityCase& test)
{
    ...
    // The current transport operator is first-order upwind. "bounded" is
    // conservative continuity correction, not a second-order LUD/TVD scheme.
    ...
    return {std::move(U),solve,build_fv_geometry(mesh),std::move(mesh)};
}
                        ...
                        double coordinate)
{
    ...
    const auto at=[&](std::size_t i,std::size_t j) { return values[j*nx+i]; };
    const double v00=at(i0,j0), v10=at(i0+1,j0);
    const double v01=at(i0,j0+1), v11=at(i0+1,j0+1);
                   ...
                   std::size_t nx,std::size_t ny,bool horizontal_component,const std::vector<Sample>& samples)
{
    ...
    for(const auto& sample:samples) {
        const double error=std::abs(
            interpolate_line(U,geometry,nx,ny,horizontal_component,sample.coordinate)
            -sample.reference);
        sum2+=error*error;
        max_abs=std::max(max_abs,error);
    }
    return {std::sqrt(sum2/static_cast<double>(samples.size())),max_abs};
}
...
std::vector<Sample> u_reference(double re)
{
    ...
    for(std::size_t i=0;i<17;++i) result.push_back({y[i],values[i]});
    return result;
}

std::vector<Sample> v_reference(double re)
{
    static const double x[]={0.0000,0.0625,0.0703,0.0781,0.0938,0.1563,0.2266,0.2344,0.5000,0.8047,0.8594,0.9063,0.9453,0.9531,0.9609,0.9688,0.9766,1.0000};
    ...
    std::vector<Sample> result;
    for(std::size_t i=0;i<17;++i) result.push_back({x[i],values[i]});
    return result;
}
...
CaseMetrics run_case(const CavityCase& test)
{
    ...
    const auto flux=make_mass_flux(
        result.mesh,result.geometry,result.velocity,1.0,
        {{"top",{VelocityBoundaryCondition::Type::FIXED_VALUE,{1,0,0}}}});
    double top_normal_flux=0.0;
    for(const auto face:result.mesh.boundary().patch(1).face_ids)
        top_normal_flux=std::max(top_normal_flux,std::abs(flux(face)));
    ...
    return {u,v,result.solve.history.back().continuity_linf};
}
...
int main()
{
    try {
        ...
        const auto r400=run_case({400.0,64,64,9000});
        ...
        std::cout<<std::setprecision(6)
                 <<"GHIA Re=100 observed_order V_max="<<p_v_max
                 <<" V_RMS="<<p_v_rms
                 <<" U_max="<<p_u_max<<"\n";

        // The present transport discretization is first-order upwind, so this
        // gate deliberately requires positive convergence rather than claiming
        // second-order convergence. A future LUD/TVD implementation can raise
        // this gate to the expected second-order regime.
        if(!(p_v_max>0.25) || !(p_v_rms>0.25) || !(p_u_max>0.25))
            throw std::runtime_error("Ghia Re=100 profiles do not show mesh convergence");
