...
struct Comparison {
    double rms = 0.0;
    double max_abs = 0.0;
    double max_relative_to_lid = 0.0;
};

struct CavityResult {
    Field<double, Location::CELL> velocity;
...
{
    if (nx < 4 || ny < 4)
        throw std::invalid_argument("cavity mesh is too small");

    Mesh mesh;
    const std::size_t npoints = (nx + 1) * (ny + 1) * 2;
    mesh.points().resize(npoints);
    ...
    auto add_or_get_face = [&](std::initializer_list<std::size_t> vertices,
                               std::size_t cell) {
        std::vector<std::size_t> key(vertices);
        std::vector<std::size_t> sorted = key;
        std::sort(sorted.begin(), sorted.end());

        const auto found = face_map.find(sorted);
        if (found != face_map.end()) {
            mesh.ownership().set_neighbour(found->second,
    ...
    for (std::size_t j = 0; j < ny; ++j) {
        for (std::size_t i = 0; i < nx; ++i) {
            ...
            const std::size_t a = point_id(i,j,0);
            const std::size_t b = point_id(i+1,j,0);
            const std::size_t c = point_id(i+1,j+1,0);
            const std::size_t d = point_id(i,j+1,0);
            const std::size_t e = point_id(i,j,1);
            const std::size_t f = point_id(i+1,j,1);
            const std::size_t g = point_id(i+1,j+1,1);
            const std::size_t h = point_id(i,j+1,1);

            cell_faces[cell] = {
                add_or_get_face({a,d,c,b},cell), // z-
                add_or_get_face({e,f,g,h},cell), // z+
                add_or_get_face({a,b,f,e},cell), // y-
                add_or_get_face({d,h,g,c},cell), // y+
                add_or_get_face({a,e,h,d},cell), // x-
                add_or_get_face({b,c,g,f},cell)  // x+
            };
        }
    }
    ...
    Patch bottom;
    bottom.name = "bottom";
    bottom.type = PatchType::WALL;
    Patch left;
    left.name = "left";
    left.type = PatchType::WALL;
    Patch right;
    right.name = "right";
    right.type = PatchType::WALL;
    Patch top;
    top.name = "top";
    top.type = PatchType::WALL;
    Patch front;
    front.name = "front";
    front.type = PatchType::WALL;
    Patch back;
    back.name = "back";
    back.type = PatchType::WALL;
    ...
    mesh.boundary().add_patch(left);
    mesh.boundary().add_patch(right);
...
CavityResult solve_cavity(const CavityCase& test)
{
    ...
    if (!solve.converged)
        throw std::runtime_error("Ghia cavity did not converge for Re=" +
                                 std::to_string(test.reynolds));
...
Comparison compare(const Field<double,Location::CELL>& U,
                   const FvGeometry& geometry,
                   bool horizontal_component,
                   const std::vector<Sample>& samples)
{
    ...
    for (const auto& sample : samples) {
        const double value = interpolate_line(U,geometry,horizontal_component,sample.coordinate);
        const double error = std::abs(value-sample.reference);
        sum2 += error*error;
        max_abs = std::max(max_abs,error);
    }
    const double rms = std::sqrt(sum2/static_cast<double>(samples.size()));
    return {rms,max_abs,max_abs};
}
...
void run_case(const CavityCase& test)
{
    ...
    std::cout << "GHIA Re=" << test.reynolds
    ...
    // The 64^2 finite-volume result is deliberately required to be
    // quantitatively close to Ghia, rather than merely converged.
