...
int main()
{
    ...
    run_case("radiation_p1_source_has_full_solid_angle", [] {
        const double absorption=0.5;
        const double temperature=800.0;
        const double expected=4.0*absorption*
            blackbody_emissive_power(temperature);
        EXPECT_NEAR(
            p1_radiative_source(absorption,0.0,temperature),
