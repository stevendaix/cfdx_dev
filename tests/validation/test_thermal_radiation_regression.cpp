...
int main()
{
    ...
    run_case("thermal_enthalpy_linear_extrapolation", [] {
        ThermophysicalProperties p;
        p.heat_capacity.model=ScalarPropertyModel::TABLE;
        p.heat_capacity.table.temperature={300,400};
        p.heat_capacity.table.value={1000,1200};
        p.heat_capacity.table.extrapolation=ExtrapolationPolicy::LINEAR;
        EXPECT_NEAR(p.enthalpy(300,250),47500,1e-10);
        EXPECT_NEAR(p.enthalpy(250,300),-47500,1e-10);
    });
