...
ValidationMetrics check_case(std::size_t nz)
{
    ...
    U.fill(0.0);
    p.fill(0.0);
    // Seed the pressure field with the analytical linear pressure drop. The
    // coupled momentum predictor uses the cell pressure gradient; fixed
    // pressure values on inlet/outlet alone are not a substitute for that
    // gradient during the first predictor iteration.
    for (std::size_t cell = 0; cell < mesh.n_cells(); ++cell) {
        const std::size_t k = cell / (nr * ns);
        const double z = L * (static_cast<double>(k) + 0.5) /
    ...
    c.coupling.active_velocity_components = {false, false, true};
    ...
    if (!result.converged) {
        std::string diagnostics = "VMFL005 coupled solver did not converge";
        if (!result.history.empty()) {
            const auto& h = result.history.back();
            diagnostics += " after " + std::to_string(result.iterations) +
                " iterations: momentum_rel=" + std::to_string(h.momentum_residual) +
                " momentum_eq_rel=" + std::to_string(h.momentum_equation_residual_relative) +
