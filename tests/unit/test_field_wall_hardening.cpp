...
int main() {
    ...
    run_case("wall_function_reference_relations", [] {
        ...
        const double yp=physics::wall::y_plus(y,utau,nu);
        EXPECT_NEAR(yp,50.0,1e-12);
        EXPECT_TRUE(physics::wall::u_plus_log(yp)>0.0);
        EXPECT_TRUE(physics::wall::turbulent_viscosity_log(y,utau,nu)>0.0);
        EXPECT_TRUE(physics::wall::omega_log(y,utau)>0.0);
    });
    return run_all();
}
