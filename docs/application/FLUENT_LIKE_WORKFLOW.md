...
The application layer now covers the full UI-neutral workflow contract:
...
This is intentionally the application contract rather than a display toolkit. A Qt/VTK frontend can bind to Workbench, while an HPC/headless execution can use the same objects through CommandBus.
