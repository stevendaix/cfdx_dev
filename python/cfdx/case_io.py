...
def validate_case_bundle(path: Path) -> dict[str, bool]:
    """Validate that one CFDX HDF5 artifact contains case, mesh and fields.

    The native mesh schema is intentionally checked by dataset presence here;
    the C++ HDF5 reader remains the authoritative topology validator. This
    function is the application-layer gate that prevents a case-only file from
    being mistaken for a self-contained CFDX case.
    """
    ...
    with h5py.File(case_path, "r") as h5:
        ...
        if "mesh" not in h5 and not all(name in h5 for name in required_mesh):
    ...
    return {
        ...
        "physics": True,
        "numerics": True,
        "runtime_separated": runtime_separated,
    }
