...
struct GmresWorkspace{
 std::size_t n=0,restart=0;Vector r,w,z,vin,vout,ax;std::vector<double> basis,zbasis,h,cs,sn,g,y;
 void resize(std::size_t n_,std::size_t m_){n=n_;restart=m_;r.resize(n);w.resize(n);z.resize(n);vin.resize(n);vout.resize(n);ax.resize(n);basis.assign((m_+1)*n_,0.0);zbasis.assign(m_*n_,0.0);h.assign((m_+1)*m_,0.0);cs.assign(m_,0.0);sn.assign(m_,0.0);g.assign(m_+1,0.0);y.assign(m_,0.0);}
