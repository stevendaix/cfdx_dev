...
// The preconditioner applies one forward triangular sweep to M z = r,
// where M is the lower-triangular part including the diagonal.
    ...
    bool setup(const SparseMatrix& A) override {
        ...
        for (std::size_t i = 0; i < n_; ++i) {
            for (std::size_t k = ro[i]; k < ro[i + 1]; ++k) {
                ...
                if (j == i) diag_[i] = av[k];
                else if (j < i) lower_rows_[i].push_back({j, av[k]});
            }
            if (std::abs(diag_[i]) <= std::numeric_limits<double>::epsilon()) {
                clear();
                return false;
            }
        }
...
public:
    bool setup(const SparseMatrix& A) override {
        if (A.n_rows() != A.n_cols() || !A.is_consistent()) return false;
        n_ = A.n_rows();
        row_offsets_.assign(A.row_offsets_data(),
                             A.row_offsets_data() + n_ + 1);
        columns_.assign(A.columns_data(),
                        A.columns_data() + A.nnz());
        ...
        for (std::size_t i = 0; i < n_; ++i) {
            for (std::size_t k = row_offsets_[i]; k < row_offsets_[i + 1]; ++k)
                if (columns_[k] == i) diag_pos_[i] = k;
            if (diag_pos_[i] == invalid_) {
                clear();
                return false;
            }
        }
        ...
        for (std::size_t i = 0; i < n_; ++i) {
            for (std::size_t kk = row_offsets_[i]; kk < row_offsets_[i + 1]; ++kk) {
                ...
                if (std::abs(lu_[dk]) <= std::numeric_limits<double>::epsilon()) {
                    clear();
                    return false;
                }
                lu_[kk] /= lu_[dk];
                ...
                for (std::size_t kj = row_offsets_[kcol]; kj < row_offsets_[kcol + 1]; ++kj) {
                    ...
                    if (pos != invalid_) lu_[pos] -= lik * lu_[kj];
                }
            }
            if (std::abs(lu_[diag_pos_[i]]) <= std::numeric_limits<double>::epsilon()) {
                clear();
                return false;
            }
        }
