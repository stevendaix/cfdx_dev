...
inline std::uint64_t morton3d(std::uint32_t x,std::uint32_t y,std::uint32_t z){
 auto split=[](std::uint32_t v) -> std::uint64_t { std::uint64_t r=0; for(unsigned i=0;i<21;++i) r|=((static_cast<std::uint64_t>(v)>>i)&1ULL)<<(3*i); return r; };
...
inline std::vector<std::uint32_t> morton_order(const std::vector<Vec3>& centres){
 ...
 auto q=[&](double v,double a,double b) -> std::uint32_t { if(b<=a) return 0U; const double t=std::clamp((v-a)/(b-a),0.0,1.0); return static_cast<std::uint32_t>(t*static_cast<double>((1U<<21)-1U)); };
