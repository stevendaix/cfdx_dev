...
enum class ExecutionPolicy : std::uint8_t { CPU = 0, GPU, GPU_OUT_OF_CORE, AUTO };
...
enum class CpuExecutionMode : std::uint8_t { SERIAL = 0, PARALLEL };
...
inline std::size_t resolve_cpu_threads(
    const CpuExecutionConfig& config, const RuntimeCapabilities& caps)
{
...
inline void validate_gpu_device(
    const GpuExecutionConfig& config, const RuntimeCapabilities& caps)
{
    ...
    const GpuExecutionConfig& gpu_config)
{
    ...
    auto select_cpu = [&] {
        if (!caps.cpu)
            throw std::runtime_error("CPU backend is unavailable and no fallback is permitted");
    ...
    auto select_gpu = [&](bool out_of_core) {
        ...
        if (!out_of_core && !fits)
            throw std::runtime_error("GPU policy requested but workload exceeds GPU memory budget");
        if (out_of_core && !gpu_config.allow_out_of_core)
            throw std::runtime_error("GPU workload exceeds memory budget and GPU out-of-core is disabled");
...
inline RuntimeDecision choose_execution_policy(
    ...
    const CpuExecutionConfig& cpu_config)
{
    return choose_execution_policy(requested, caps, workload, cpu_config, GpuExecutionConfig{});
}
